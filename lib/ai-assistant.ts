...
// To use real Grok API, install: pnpm add @xai/sdk
// And set NEXT_PUBLIC_XAI_API_KEY in .env

const USE_LIVE_AI = process.env.NEXT_PUBLIC_XAI_API_KEY && process.env.NEXT_PUBLIC_USE_LIVE_AI === 'true';
...
export async function parseVoiceCommand(transcript: string): Promise<AICommand> {
  if (USE_LIVE_AI) {
    ...
    // const client = new Client({ apiKey: process.env.NEXT_PUBLIC_XAI_API_KEY });
    // const response = await client.chat.completions.create({
    //   model: 'grok-beta',
    //   messages: [{
    //     role: 'user',
    //     content: `Parse this keg tracking command and return JSON: "${transcript}". 
    //     Possible actions: create_keg, batch_create, assign_driver, view_dashboard, scan_keg, end_shift, generate_report.
    //     Return format: {action: string, params: object, confidence: number}`
    //   }],
    // });
  ...
  if (lower.includes('create') || lower.includes('new') || lower.includes('mint')) {
    ...
    if (batchMatch && parseInt(batchMatch[1]) > 1) {
      return {
        action: 'batch_create',
        params: { ...params, count: parseInt(batchMatch[1]) },
        confidence: 0.85
      };
    }
