...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Get all active kegs
    const { data: activeKegs, error: kegsError } = await supabase.from("kegs").select("id").eq("is_empty", false)
    ...
    return NextResponse.json({ error: "Failed to sync POS data" }, { status: 500 })
  }
}
