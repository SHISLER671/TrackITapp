...
export async function POST(request: NextRequest) {
  ...
  if (userRole.role !== 'DRIVER') {
    return NextResponse.json(
      { error: 'Only drivers can create deliveries' },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (!kegs || kegs.length !== validatedData.keg_ids.length) {
      return NextResponse.json(
        { error: 'One or more kegs not found' },
        { status: 404 }
      );
    }
    ...
    if (invalidKegs.length > 0) {
      return NextResponse.json(
        { error: 'You can only deliver kegs currently in your possession' },
        { status: 403 }
      );
    }
    ...
    if (breweryIds.length > 1) {
      return NextResponse.json(
        { error: 'All kegs must be from the same brewery' },
        { status: 400 }
      );
    }
