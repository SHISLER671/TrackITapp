...
export async function POST(
  ...
  try {
    ...
    const { notes, delivery_proof } = body
    ...
    if (!['IN_TRANSIT', 'PENDING'].includes(delivery.status)) {
      return NextResponse.json(
        { error: 'Delivery cannot be accepted in current status' },
        { status: 400 }
      )
    }
    ...
    // Update keg statuses to delivered
    if (delivery.delivery_items && delivery.delivery_items.length > 0) {
      ...
      // Create keg scans for delivered kegs
      const scanRecords = kegIds.map((kegId: string) => ({
        keg_id: kegId,
        location: delivery.restaurant_id,
