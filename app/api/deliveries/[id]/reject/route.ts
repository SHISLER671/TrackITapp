...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (userRole.role !== "RESTAURANT_MANAGER") {
    return NextResponse.json({ error: "Only restaurant managers can reject deliveries" }, { status: 403 })
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to reject delivery" }, { status: 500 })
  }
}
