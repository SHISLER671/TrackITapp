...
function NewDeliveryContent() {
  ...
  const handleKegToggle = (kegId: string) => {
    setSelectedKegIds((prev) => (prev.includes(kegId) ? prev.filter((id) => id !== kegId) : [...prev, kegId]))
  }
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {kegsOnTruck.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No kegs on truck</h3>
            <p className="text-gray-600 mb-6">Scan kegs to load them onto your truck first</p>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <label className="block text-sm font-medium text-gray-700 mb-3">Destination Restaurant *</label>
              <select
              ...
              <p className="mt-2 text-sm text-gray-500">{restaurants.length} restaurant(s) available</p>
            </div>

            {/* Keg Selection */}
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center justify-between mb-4">
                <label className="block text-sm font-medium text-gray-700">Select Kegs for Delivery *</label>
                <button type="button" onClick={handleSelectAll} className="text-sm text-blue-600 hover:underline">
                  {selectedKegIds.length === kegsOnTruck.length ? "Deselect All" : "Select All"}
                </button>
              </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="ml-4 flex-1">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-sm font-medium text-gray-700">$30 deposit</div>
                          </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                <div className="flex justify-between items-center">
                  <span className="font-medium text-gray-900">Selected: {selectedKegIds.length} keg(s)</span>
                  <span className="text-lg font-bold text-blue-600">Total Deposit: ${totalDeposit.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            {/* Notes */}
            <div className="bg-white rounded-lg shadow p-6">
              <label className="block text-sm font-medium text-gray-700 mb-3">Delivery Notes (Optional)</label>
              <textarea
                value={notes}
